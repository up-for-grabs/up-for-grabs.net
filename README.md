...
* We are using a file called `scripts.html` which is an include file. When we publish the site, Jekyll uses this to concatenate and generate the scripts that will be downloaded when a user visits the site.
...
Recent examples :
 ...
 - Cleanup stale projects - each week a task runs to review the list of projects
   and remove any that have been marked as archived or are no longer accessible via the GitHub API.
