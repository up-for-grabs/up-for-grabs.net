...
def check_folder
  all_files =  Dir["../_data/projects/*"]
  yaml_files =  Dir["../_data/projects/*.yml"]
...
results = Dir["../_data/projects/*.yml"].map { |f| verify_file(f) }

files_with_errors = results.select { |file, error| error != nil }
error_count = files_with_errors.count

success = results.select { |file, error| error.nil? }
                 .count

puts "#{success} files processed"

if (error_count > 0) then
  puts "#{error_count} files with errors!"
