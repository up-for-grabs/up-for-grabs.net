...
async function search() {
  const response = await SearchProjects(searchText.value, lastUpdatedDate.value);
...
<template>
    <menu>
        <form class="form-wrapper">
            <label id="search-by-text">
              Search projects by text...
            </label>
            <input type="text" id="search" aria-labelledby="search-by-text" v-on:input="onTextChanged" placeholder="Enter text to filter projects..." />
            <div>
              <label id="activity-filter">Choose active projects active within the previous</label>

              <select v-on:change="onPeriodChanged" aria-labelledby="activity-filter">
                <option value="7">1 week</option>
                <option selected value="30">1 month</option> <!-- TODO: how to keep selected in sync with items? onMount? -->
    ...
    <div class="results-count" aria-live="polite">{{ currentProjects.length }} projects found</div>
    <div class="projects">
        <ProjectEntry v-for="project in currentProjects" :key="project.id" :project="project" />
    </div>
</template>